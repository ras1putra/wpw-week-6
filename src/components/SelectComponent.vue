<template>
    <div class="mx-4 p-4 flex justify-between font-bold items-center">
        <label>{{ label }}</label>
        <select class="w-[73%] rounded-md px-4 py-2 text-violet-700" :value="modelValue" @change="$emit('update:modelValue', $event.target.value)" v-bind="$attrs" :class="{ 'is-invalid': getError }">
